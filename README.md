...
1. **使用社区补丁版 Python 3.8.20** (XP 兼容，来自 PythonVista 项目)
2. 使用 PyInstaller 4.10
3. 使用 32 位 Python
...
项目已配置好 GitHub Action，会自动使用社区补丁版 Python 3.8.20 构建 XP 兼容的 exe。
...
vpid_fyne/
├── .github/workflows/
│   └── build-release.yml    # GitHub Action 自动发布配置 (Python 3.8.20)
├── src/
│   ├── __init__.py
...
- 使用社区补丁版 Python 3.8.20 (XP 兼容，来自 PythonVista 项目)
- 使用纯 Python 内置模块 (`winreg`, `wmi`)
- 不依赖 Windows Vista+ 新增 API
- 使用 Tkinter 标准 GUI 框架
- 使用 PyInstaller 4.10 打包
- GitHub Action 自动使用社区补丁版 Python 3.8.20 构建

## XP 兼容 Python 版本

推荐 XP 兼容的 Python 版本：

- **Python 3.8.20 社区补丁版** (来自 PythonVista 项目，推荐)
- Python 3.5 (最后官方支持 XP 的版本，已停止维护)
